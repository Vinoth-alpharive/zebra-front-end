...
const Farmimg = () => {
  ...
  const farmingPairs = async () => {
    try {
      const { data } = await Axios.post(`/admin/farmingLiquidity`, { contract: "all" })
  ...
  const balances = async (ele) => {
    try {
      ...
      // console.log(await WEB.utils.fromWei(balance, 'ether'), "bal")
      setBalance(await WEB.utils.fromWei(balance, 'ether'))
    } catch (error) {
      console.log("🚀 error:", error)
  ...
  const unStakes = async (element) => {
    try {
      ...
      // const al = await WEB.utils.fromWei(userInfo.amount, 'ether')
      const withdraw = await routeInstance.methods.withdraw(element?.ID, userInfo.amount).send({ from: address[0] })
      if (withdraw) {
        farmingPairs()
  ...
  const farming = async (selc) => {
    try {
      ...
      if (Number(amount.current.value) <= 0) {
        ...
        const am = await WEB.utils.toWei(amount.current.value, 'ether')
        // approve
        ...
        const approve = await erc20Instance.methods.approve(selc?.contract_Address, am).send({
          from: address[0]
        })
        // ****************************************************************
        if (approve) {
          // deposite
          const amou1 = await routeInstance.methods.deposit(selc?.ID, am).send({ from: address[0] })
          if (amou1) {
            handleCloseliq()
            farmingPairs()
            await Axios.post(`/users/createTrade`, {
              ...
              <Box sx={{ minWidth: 120 }}>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    onChange={sortby}
                  >
                    <MenuItem>APR</MenuItem>
                    <MenuItem>Earned</MenuItem>
                      ...
                      {isActive === index &&
                        <>
                          ...
                          <TableCell className='w-60-percent' colspan='8 '>
                            {already.includes(element?._id) === false ?
                              <>
